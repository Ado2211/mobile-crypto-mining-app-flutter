...
class MyConverter {
  ...
  static String getReturnPerDay(String min,String max,{int precision=2}) {
    try {
      double maximum = double.parse(max);
      double minimum = double.parse(min);
      if(maximum==minimum){
        return minimum.toStringAsFixed(minimum.truncateToDouble() == minimum ? 0 : 2);
      } else{
        return '${minimum.toStringAsFixed(minimum.truncateToDouble() == minimum ? 0 : 2)} - ${maximum.toStringAsFixed(maximum.truncateToDouble() == maximum ? 0 : 2)}';
      }
    } catch (e) {
      return min;
    }
